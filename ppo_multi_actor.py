...
def ppo(env, max_iter_count: int = 10000,
    ...
    for _ in tqdm(range(max_iter_count)):
        ...
        for i, cenv in enumerate(envs):
            if cenv.is_game_over():
                if first_episode:
                    ema_score = cenv.score()
                    first_episode = False
                else:
                    ema_score = (1 - 0.999) * cenv.score() + 0.999 * ema_score
                    ema_score_progress.append(ema_score)

                cenv.reset()

            s = cenv.state_description()

            aa = cenv.available_actions_ids()

            mask = np.zeros((cenv.max_action_count(),))
            mask[aa] = 1.0
        ...
        for i, cenv in enumerate(envs):
            old_score = cenv.score()
            cenv.act_with_action_id(chosen_actions[i])
            new_score = cenv.score()
            r = new_score - old_score

            rewards[i] = r

            s_p = cenv.state_description()
            aa_p = cenv.available_actions_ids()

            mask_p = np.zeros((cenv.max_action_count(),))
